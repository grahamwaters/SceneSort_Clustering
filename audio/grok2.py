...
EXPECTED_FEATURE_DIM = 42
SILHOUETTE_THRESHOLD = 0.3  # Below this, try to improve
MAX_RETRIES = 3  # Limit re-clustering attempts
...
def extract_features(file_path):
    try:
        ...
        try:
            ...
            tempo = tempo_array[0] if tempo_array.size > 0 else 0.0
...
# Find optimal number of clusters or test range
def optimize_clusters(features_reduced, initial_k, max_k=MAX_CLUSTERS_DEFAULT, min_k=MIN_CLUSTERS):
    if len(features_reduced) < min_k:
        return min_k if len(features_reduced) > 1 else 1, None, None

    max_k = min(int(np.sqrt(len(features_reduced))) + 1, max_k + 1, len(features_reduced))
    best_k = initial_k
    best_score = -1
    best_labels = None
    ...
    for attempt in range(MAX_RETRIES + 1):
        silhouette_scores = []
        k_range = range(max(min_k, 2), max_k)
        if attempt > 0:
            console.print(f"[yellow]Silhouette score {best_score:.3f} below threshold {SILHOUETTE_THRESHOLD}. Retrying (attempt {attempt}/{MAX_RETRIES})...[/yellow]")

        for k in k_range:
            kmeans = KMeans(n_clusters=k, random_state=42, n_init=10)
            labels = kmeans.fit_predict(features_reduced)
            score = silhouette_score(features_reduced, labels) if k > 1 else 1.0
            silhouette_scores.append((k, score, labels))

        current_best = max(silhouette_scores, key=lambda x: x[1])
        if current_best[1] > best_score:
            best_k, best_score, best_labels = current_best

        if best_score >= SILHOUETTE_THRESHOLD or attempt == MAX_RETRIES:
            break

        # Adjust k range for next attempt: try increasing or decreasing based on current best
        if best_k == max(k_range) - 1:
            min_k = best_k
            max_k = min(max_k + 2, len(features_reduced))
        elif best_k == min(k_range):
            max_k = best_k + 1
            min_k = max(MIN_CLUSTERS, best_k - 2)
        else:
            min_k = max(MIN_CLUSTERS, best_k - 1)
            max_k = min(max_k, best_k + 2)
...
def process_audio_files():
    ...
    # Determine initial optimal number of clusters
    console.print("\n[bold magenta]Determining Initial Optimal Cluster Count...[/bold magenta]")
    initial_k = max(MIN_CLUSTERS, min(int(np.sqrt(len(features_reduced))) + 1, MAX_CLUSTERS_DEFAULT))
    optimal_k, silhouette_avg, labels = optimize_clusters(features_reduced, initial_k)
    ...
    tempo_idx = 13 + 12 + 4  # Index 29
    rms_idx = 13 + 12 + 2    # Index 27
