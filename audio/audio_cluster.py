    ...
    try:
        ...
        try:
            ...
            features = np.concatenate((
                ...
                [spec_centroid, spec_flatness, spec_contrast, zero_crossing, rmse, tempo, onset_env],
                tonnetz
            ))
        except Exception as e:
...
def find_optimal_clusters(features_scaled, max_k=MAX_CLUSTERS):
    if len(features_scaled) < 2:
        return 1
    silhouette_scores = []
    max_k = min(max_k + 1, len(features_scaled))
...
def process_audio_files():
    ...
    rmse_idx = feature_dim - 8
    for cluster_id in sorted(clusters.keys()):
        cluster_features = [f for i, f in enumerate(features) if labels[i] == cluster_id]
        avg_tempo = np.mean([f[tempo_idx] for f in cluster_features]) if len(cluster_features) > 0 else 0
        avg_energy = np.mean([f[rmse_idx] for f in cluster_features]) if len(cluster_features) > 0 else 0
