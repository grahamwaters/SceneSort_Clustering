#!/usr/bin/env python3
"""
Enhanced Media Scene Sorter with Hybrid Clustering and iCloud RAW Copy

Extended Summary:
- Scans your iCloud folder for RAW images (.rw2 and other raw formats), forces each to download if needed,
  and copies them to an external drive.
- Once the images are on the external drive, it performs analysis by clustering files based on visual and temporal features.
- Supports resuming from a previously saved progress file if interrupted.
- Optionally skips video files.
...
def ensure_file_downloaded(file: Path, min_size: int = 1024 * 1024):
    """
    Ensure the iCloud file is fully downloaded.
    If the file size is less than min_size (default 1MB), it triggers a download using 'brctl download'.
    """
    try:
        size = file.stat().st_size
    except Exception as e:
        logger.error("Could not stat file %s: %s", file, e)
        return
    if size < min_size:
        logger.info("File %s appears not fully downloaded (size: %d bytes). Forcing download...", file, size)
        try:
            subprocess.run(["brctl", "download", str(file)], check=True)
            time.sleep(2)  # wait a short period to allow the download to complete
            new_size = file.stat().st_size
            if new_size < min_size:
                logger.warning("File %s still appears small after download attempt (%d bytes)", file, new_size)
            else:
                logger.info("File %s successfully downloaded (%d bytes)", file, new_size)
        except Exception as e:
            logger.error("Failed to force download for %s: %s", file, e)


def copy_icloud_raw_images(icloud_path: Path, destination_path: Path):
    """
    Traverse the iCloud folder and copy any RAW images (.rw2, .cr2, etc.)
    to the external drive folder. This function forces a download for each file if necessary.
    """
    if not icloud_path.exists() or not icloud_path.is_dir():
        logger.error(f"iCloud path {icloud_path} does not exist or is not a directory.")
        return

    destination_path.mkdir(parents=True, exist_ok=True)
    raw_files = []
    logger.info("Scanning iCloud folder for RAW images...")
    for file in tqdm(list(icloud_path.rglob('*')), desc="Scanning iCloud"):
        if file.is_file() and file.suffix.lower() in RAW_EXTENSIONS:
            # Ensure the file is downloaded from iCloud.
            ensure_file_downloaded(file)
            raw_files.append(file)

    logger.info("Found %d RAW files in iCloud.", len(raw_files))
    for file in tqdm(raw_files, desc="Copying RAW images"):
        try:
            dest_file = get_unique_dest_path(destination_path, file.name)
...
class MediaProcessor:
    def __init__(self, config: dict):
        ...
        self.model_lock = None  # For thread safety (if needed)
    ...
    def _gather_files(self, input_dir: Path) -> List[Path]:
        ...
        media_files = []
        allowed_exts = IMAGE_EXTENSIONS | RAW_EXTENSIONS
        if self.config['processing'].get('include_videos', True):
    ...
    def _extract_features(self, files: List[Path]) -> Tuple[List[Path], List[Tuple[np.ndarray, float]]]:
        """Extract visual and temporal features from the files."""
        processed_files = []
        features = []
    ...
    def _organize_files(self, files: List[Path], clusters: np.ndarray, output_dir: Path):
        ...
        for path, cluster_id in zip(files, clusters):
            ...
            dest_path = get_unique_dest_path(dest_dir, path.name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', type=Path, default=None, help="Input directory")
    ...
    parser.add_argument('--skip-videos', action='store_true',
                        help="Skip processing video files")
    args = parser.parse_args()

    # Hard-coded iCloud and destination paths.
    # icloud_path = Path("/Users/watersterminal/Library/Mobile Documents/comappleCloudDocs/")
    icloud_path = Path("")
    destination_path = Path("/Volumes/BigBoy/ShutterStockContributor")
    if args.input is None:
        args.input = destination_path  # Use destination folder as input for analysis
    if args.output is None:
        args.output = destination_path

    logger.info("Starting iCloud RAW image copy...")
    copy_icloud_raw_images(icloud_path, destination_path)
    logger.info("Completed iCloud RAW image copy.")
    ...
    config.setdefault('processing', {})['include_videos'] = not args.skip_videos
